...
let fullNames = [];
runners.forEach (i => fullNames.push(`${i.first_name} ${i.last_name}`));
...
firstNamesAllCaps = runners.map(i => i.first_name.toUpperCase());
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
let runnersLargeSizeShirt = [];
runnersLargeSizeShirt = runners.filter (i => i.shirt_size === 'L');
...
// Problem 1
// Array of first name and emails.
const emailList = [];
runners.forEach(i => {emailList.push([i.first_name, i.email])});

console.log(emailList);


// Problem 2
// Only runners from company "Jaxbean"
const JaxbeanRunners = runners.filter(i => i.company_name === 'Jaxbean');

console.log(JaxbeanRunners);
